        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="space-y-4">
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-slate-950 font-serif font-bold text-lg">
                {user?.fullName ? user.fullName[0].toUpperCase() : 'C'}
              </div>
              <div>
                <h3 className="text-sm font-semibold text-slate-100">{user?.fullName || 'Legal Counsel'}</h3>
            ...
            <div className="pt-3 border-t border-slate-800 text-xs space-y-2">
              <div className="flex justify-between">
                ...
                <span className="text-amber-300 font-semibold">Enterprise Counsel</span>
              </div>
              <div className="flex justify-between">
          ...
          <Card className="md:col-span-2 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1 text-xs">
              <div className="p-3 bg-[#0d1017] rounded-xl border border-slate-800 space-y-1">
                <span className="font-semibold text-slate-200 block">Row-Level SQL Scoping</span>
                <p className="text-[11px] text-slate-400">All queries parameterized strictly with verified JWT userId.</p>
              </div>
              <div className="p-3 bg-[#0d1017] rounded-xl border border-slate-800 space-y-1">
                <span className="font-semibold text-slate-200 block">pgvector Cosine Isolation</span>
                <p className="text-[11px] text-slate-400">Embeddings never pooled globally across foreign matters.</p>
              </div>
              <div className="p-3 bg-[#0d1017] rounded-xl border border-slate-800 space-y-1">
                <span className="font-semibold text-slate-200 block">Prompt Injection Guard</span>
                <p className="text-[11px] text-slate-400">All parsed legal filings treated as untrusted document text.</p>
              </div>
              <div className="p-3 bg-[#0d1017] rounded-xl border border-slate-800 space-y-1">
                <span className="font-semibold text-slate-200 block">Zero-Hallucination Citations</span>
                    ...
                    {logs.map((log) => (
                      <tr key={log.id} className="hover:bg-white/5 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <span className="font-semibold text-amber-300 font-mono px-2 py-0.5 rounded bg-amber-500/10 border border-amber-500/20">
                            {log.action}
                          </span>
                        </td>
